...
DS_PATH = '../Sec/v'
UITE_NAME = "print_tokens2"
CASE_NUM = 4115  # 测试用例数 total counted to 4115 cases
VERSION_NUM = 10  # 测试程序数 total counted to 10 versions
    ...
    for i in range(1, VERSION_NUM + 1):  # 测试程序数
        ...
        while countrow < count_lines:  # 当前行数
            ...
            if countrow != 0:
                ...
                y = passeds / passed
                z = x + y
                unpasseds = passed - passeds
                unfaileds = failed - faileds
                if z != 0:
                    tar = x / z
                    t = (unpasseds/passed)/(unfaileds/failed+unpasseds/passed)
                    if t != 0:
                        sec = tar * t
                SEC.append(sec)
            countrow += 1
        ...
        while countr < count_lines:
            if countr == 0:
                ...
                tmp.write(
                    str(int(Matrix[countr, 0])) + "\t" + str(round(SEC[countr - 1],2)) + "\n")
            countr += 1
        tmp.close()
    print("finished")
